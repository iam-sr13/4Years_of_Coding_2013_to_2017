...
void file::enc2()
{
	...
	for(i=0;i<data.size();i++) {			
		temp<<(10000000000+binary(int_(data[i])));
		t+=temp.str();
		temp.str("");		
	}
	data=t;	
...
void file::dec2()
{
	...
	string t;
	for(i=0;i<data.size();) {
		for(c=1,x=0;c<=11;c++,i++) {			
			a=data[i]-48;			
			x=x*10+a;			
		}		
		t+=char(decimal(x-10000000000));
	}
	data=t;
}
...
void file::encrypt()
{
	...
	cout<<data;
	enc2();	
	cout<<"\n\nYour double encrypted DATA:=         (press any key)\n\n";
	getch();
	cout<<data;
}
...
void file::decrypt()
{
	...
	cout<<"\n\nYour double decrypted DATA:=         (press any key)\n\n";
	getch();
	cout<<data;	
}	
	...
	for(;;) {
		...
		switch(ch) {
			case 1:	encode.getfname();
					encode.retrieve(enc);
					cout<<"Press enter to start encryption!";
					getch();
					encode.encrypt();
